...
export interface ChildCategoryProps {
    ...
    excludedCategories: Record<string, boolean>;
    setExcludedCategory(category: string, excluded: boolean): void;
}
...
export function ChildCategory({
    ...
    const isExcluded = excludedCategories[categoryPath];
...
export function ChildCategories({
    ...
    const countExcluded = childCategories.filter(
        (x) => excludedCategories[`${path}${x}`]
    ).length;
